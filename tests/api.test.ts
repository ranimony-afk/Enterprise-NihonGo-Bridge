 ...
 * Covers Phase 1 to Phase 9, Phase 3 Decks, Phase 5 News, Phase 6 Gamification, and Phase 7 Mobile API
 */
...
test("cms section registry includes all reusable page sections", () => {
  const keys = SECTION_TYPES.map((s) => s.key);
  for (const required of ["hero", "featured_courses", "jlpt", "daily_vocab", "daily_kanji", "news", "downloads", "faqs", "testimonials", "cta", "social_links"]) {
    assert.ok(keys.includes(required), `missing reusable section type: ${required}`);
  }
  assert.equal(sectionKindFor("hero"), "hero");
  assert.equal(sectionKindFor("downloads"), "download");
  assert.equal(sectionKindFor("unknown_key"), "generic");
});

test("cms editorial module statuses include draft/preview/published/archived", () => {
  for (const s of ["draft", "preview", "published", "archived"]) {
    assert.ok(isCmsStatus(s));
  }
  assert.equal(isCmsStatus("pending"), false);
});

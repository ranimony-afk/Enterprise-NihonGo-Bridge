...
export default async function BrandCmsEditor({
  ...
  if (tab === "logs") {
    ...
    grammarRulesList = await db
      .select()
      .from(grammarRules)
      .where(eq(grammarRules.brandId, brand.id))
      .limit(50);
  } else if (tab === "quizzes") {
    quizzesList = q
      ? await db
    ...
    couponsList = await db
      .select()
      .from(coupons)
      .where(eq(coupons.brandId, brand.id))
      .limit(50);
          ...
          <main className="md:col-span-3 space-y-6">
            ...
            {tab === "analytics" && (
              <div className="space-y-6 text-left">
                {/* 1. Core KPIs Metric Grid */}
                <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
                  {[
                    { title: "Total Users Enrolled", val: "12,480", inc: "+8.3%", color: "text-emerald-500" },
                    { title: "Monthly Recurring Revenue (MRR)", val: "$24,560", inc: "+15.4%", color: "text-emerald-500" },
                    { title: "Avg Quiz Accuracy", val: "91.5%", inc: "+2.0%", color: "text-rose-500" },
                    { title: "Active Daily Study Time", val: "158,400 min", inc: "+4.5%", color: "text-indigo-500" },
                  ].map((stat, idx) => (
                ...
                {/* 2. Interactive Charts: Business Revenue & Learning XP Curves */}
                <div className="grid gap-6 md:grid-cols-12">
                  {/* Learning Progress Chart (XP) */}
                  <div className={`rounded-3xl p-6 border shadow-2xs space-y-4 md:col-span-6 ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                  ...
                  {/* Business Revenue & Gateway Distribution (Stripe vs Razorpay) */}
                  <div className={`rounded-3xl p-6 border shadow-2xs space-y-4 md:col-span-6 ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                    <div className="flex justify-between items-center">
                      <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Business Revenue &amp; Gateways</h3>
                      <span className="text-[10px] font-bold text-slate-400">CGST/SGST Tax Deducted</span>
                    </div>
                    <div className="h-44 flex items-end gap-3 pt-6 pb-2 border-b border-black/10">
                      {[12, 14, 18, 16, 22, 25, 24, 28, 30, 29, 32, 35].map((rev, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                          <div
                            className="w-full bg-emerald-500 rounded-t-lg transition hover:bg-emerald-400 cursor-pointer relative"
                            style={{ height: `${(rev / 40) * 100}%` }}
                          >
                            <span className="absolute -top-7 left-1/2 -translate-x-1/2 bg-slate-900 text-[9px] font-bold text-white px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap">
                              ${rev}k ARR
                            </span>
                ...
                {/* 3. Retentions Cohort & Onboarding Funnels */}
                <div className="grid gap-6 md:grid-cols-2">
                  {/* Onboarding Funnels Layout */}
                  <div className={`rounded-3xl p-6 border shadow-2xs space-y-4 ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">B2C/B2B Conversion Funnel</h3>
                    <div className="space-y-3 pt-2 text-xs">
                      {[
                        { step: "1. Total Onboarded Learners", val: "12,480 users", ratio: "100%", w: "w-full", col: "bg-indigo-500" },
                        { step: "2. Active Daily Practice (LMS/SRS)", val: "9,734 users", ratio: "78.0%", w: "w-[78%]", col: "bg-sky-500" },
                        { step: "3. Premium Course Purchases", val: "1,920 sales", ratio: "15.4%", w: "w-[15.4%]", col: "bg-rose-500" },
                        { step: "4. Certified N5-N1 Graduates", val: "1,024 students", ratio: "8.2%", w: "w-[8.2%]", col: "bg-emerald-500" }
                      ].map((item) => (
                        <div key={item.step} className="space-y-1">
                          <div className="flex justify-between font-bold text-slate-700 dark:text-slate-300">
                            <span>{item.step}</span>
                            <span>{item.val} ({item.ratio})</span>
                          </div>
                          <div className="w-full bg-slate-100 dark:bg-slate-950 h-3 rounded-full overflow-hidden">
                            <div className={`h-full ${item.col} ${item.w} rounded-full`} />
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {/* Retention Cohort Decay Card */}
                  <div className={`rounded-3xl p-6 border shadow-2xs space-y-4 ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                    <div className="flex justify-between items-center">
                      <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">User Cohort Retention Drop-off</h3>
                      <span className="text-[10px] text-emerald-500 font-bold">Excellent Stickiness</span>
                    </div>
                    <div className="overflow-x-auto text-xs">
                      <table className="w-full text-center border-collapse">
                        <thead>
                          <tr className={`border-b ${isDark ? "border-slate-800 text-slate-400" : "border-slate-200 text-slate-500"} font-bold`}>
                            <th className="py-2.5 text-left">Cohort Month</th>
                            <th className="py-2.5">Week 0</th>
                            <th className="py-2.5">Week 1</th>
                            <th className="py-2.5">Week 2</th>
                            <th className="py-2.5">Week 3</th>
                          </tr>
                        </thead>
                        <tbody>
                          {[
                            { month: "Jan 2026 Cohort", w0: "100%", w1: "95%", w2: "84%", w3: "72%" },
                            { month: "Feb 2026 Cohort", w0: "100%", w1: "94%", w2: "82%", w3: "70%" },
                            { month: "Mar 2026 Cohort", w0: "100%", w1: "96%", w2: "85%", w3: "75%" }
                          ].map((c) => (
                            <tr key={c.month} className={`border-b ${isDark ? "border-slate-800" : "border-slate-100"}`}>
                              <td className="py-3 text-left font-bold text-slate-700 dark:text-slate-300">{c.month}</td>
                              <td className="py-3 font-bold bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 rounded">{c.w0}</td>
                              <td className="py-3 font-bold bg-emerald-500/15 text-emerald-700 dark:text-emerald-400">{c.w1}</td>
                              <td className="py-3 font-bold bg-emerald-500/10 text-emerald-700 dark:text-emerald-500">{c.w2}</td>
                              <td className="py-3 font-bold bg-emerald-500/5 text-emerald-800 dark:text-slate-400">{c.w3}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
            ...
            {tab === "workflow" && (
              <div className="space-y-6">
                ...
                <div className={`rounded-2xl p-5 border shadow-2xs ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                  <h3 className="text-sm font-bold mb-4">🔔 Live Editorial Notifications &amp; System Alerts</h3>
                  <div className="overflow-x-auto text-xs">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className={`border-b ${isDark ? "border-slate-800 text-slate-400" : "border-slate-200 text-slate-500"} font-bold`}>
                          <th className="py-2.5">Alert Message</th>
                          <th className="py-2.5">Type</th>
                          <th className="py-2.5">Recipient ID</th>
                          <th className="py-2.5">Created At</th>
                        </tr>
                      </thead>
                      <tbody>
                        {notificationsList.map((notif) => (
                          <tr key={notif.id} className={`border-b ${isDark ? "border-slate-800 hover:bg-slate-850" : "border-slate-100 hover:bg-slate-50"}`}>
                            <td className="py-2.5 font-bold text-slate-900 dark:text-white">{notif.message}</td>
                            <td className="py-2.5 font-mono text-slate-400 font-semibold">{notif.type}</td>
                            <td className="py-2.5 font-mono font-semibold text-slate-700 dark:text-slate-300">User #{notif.recipientId || "All"}</td>
                            <td className="py-2.5 text-slate-400">{notif.createdAt.toLocaleString()}</td>
                          </tr>
                        ))}
                        {notificationsList.length === 0 && (
                          <tr>
                            <td colSpan={4} className="py-4 text-center text-slate-400">
                              No editorial notification alerts logged in database. 
                            </td>
                          </tr>
                        )}
                      </tbody>
            ...
            {tab === "monetization" && (
              <div className="space-y-6">
                ...
                {/* Database-driven Transactions and Coupons Ledger */}
                <div className={`rounded-2xl p-5 border shadow-2xs ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                  <h3 className="text-sm font-bold mb-4">💳 Database Billing &amp; Transaction Ledger</h3>
                  <div className="grid gap-6 md:grid-cols-2">
                    {/* Database Coupons */}
                    <div className="space-y-2 text-xs">
                      <p className="font-bold text-slate-400 uppercase text-[9px] mb-2">Live Database Coupons ({couponsList.length})</p>
                      {couponsList.map((c) => (
                        <div key={c.id} className="rounded-xl bg-slate-50 dark:bg-slate-950 p-3 border border-black/5 flex justify-between items-center">
                          <div>
                            <p className="font-bold text-slate-950 dark:text-white font-mono">{c.code}</p>
                            <p className="text-[10px] text-slate-400">Expires: {c.expiresAt ? c.expiresAt.toLocaleString() : "Never"}</p>
                          </div>
                          <span className="rounded bg-emerald-100 dark:bg-emerald-950/40 text-emerald-800 px-2 py-0.5 font-bold">
                            {c.discountPercent}% OFF
                          </span>
                        </div>
                      ))}
                      {couponsList.length === 0 && (
                        <p className="text-slate-400 italic py-2 text-center">No coupons registered on live PostgreSQL database.</p>
                      )}
                    </div>

                    {/* Database Transactions */}
                    <div className="space-y-2 text-xs">
                      <p className="font-bold text-slate-400 uppercase text-[9px] mb-2">Live Database Transactions ({transactionsList.length})</p>
                      {transactionsList.map((t) => (
                        <div key={t.id} className="rounded-xl bg-slate-50 dark:bg-slate-950 p-3 border border-black/5 flex justify-between items-center">
                          <div>
                            <p className="font-bold text-slate-950 dark:text-white">Amount: {t.amount} {t.currency}</p>
                            <p className="text-[10px] text-slate-400 font-mono">Ref: {t.referenceId || "N/A"}</p>
                          </div>
                      ...
                      {transactionsList.length === 0 && (
                        <p className="text-slate-400 italic py-2 text-center">No payments processed on live PostgreSQL database.</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            {tab === "vocabulary" && (
              <div className="space-y-6">
                ...
                {/* 6.3 Grammar Rules Management */}
                <div className={`rounded-2xl p-5 border shadow-2xs ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
                  <h3 className="text-sm font-bold mb-4">📖 Syllabus Grammar Rules (10,000 Points Index)</h3>
                  <div className="overflow-x-auto text-xs">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className={`border-b ${isDark ? "border-slate-800 text-slate-400" : "border-slate-200 text-slate-500"} font-bold`}>
                          <th className="py-2.5">Pattern</th>
                          <th className="py-2.5">Formula / Connection</th>
                          <th className="py-2.5">Meaning</th>
                          <th className="py-2.5">JLPT Level</th>
                        </tr>
                      </thead>
                      <tbody>
                        {grammarRulesList.map((rule) => (
                          <tr key={rule.id} className={`border-b ${isDark ? "border-slate-800 hover:bg-slate-850" : "border-slate-100 hover:bg-slate-50"}`}>
                            <td className="py-2.5 font-bold text-base text-rose-700">{rule.title}</td>
                            <td className="py-2.5 font-mono text-slate-400">{rule.pattern}</td>
                            <td className="py-2.5 font-semibold text-slate-700 dark:text-slate-300">{rule.meaning}</td>
                            <td className="py-2.5 font-bold font-mono text-slate-400">{rule.level || "—"}</td>
                          </tr>
                        ))}
                        {grammarRulesList.length === 0 && (
                          <tr>
                            <td colSpan={4} className="py-4 text-center text-slate-400">
                              No custom grammar rules found in database. 10,000 standard static items are available on client-side search.
                            </td>
                          </tr>
                        )}
                      </tbody>
